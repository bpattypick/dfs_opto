...
- **T19. Captain selection must weight confidence, not just projected points — CONFIRMED TWICE LIVE.** SF@LAR captained an over-projected Stafford; DEN@KC captained J.K. Dobbins, who returned 3.6 points on zero targets in a 21-point blowout loss — the single worst game script for a non-receiving back, and the optimizer had no way to see that risk, only his mean. Two for two on the first two live pipeline lineups. Needs a floor/ceiling notion per player (from T13's fitted variance, already available) so the optimizer can prefer a lower-mean, lower-variance captain over a boom/bust one at the same salary. No longer theoretical — do this before the next live lineup, not after T13/T18 land elsewhere. *Blocks: trusting the optimizer's captain choice.*
