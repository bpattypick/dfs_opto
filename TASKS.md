...
### T23. Score the backtest on the full roster, not just stat-recorders
`player_week_stats` holds ~11 offensive players per team-game; a DK pool has ~25. The harness therefore never scores "does he play at all", the exact question the three live losses turned on, so backtest MAE understates live error by construction. Use nflverse weekly rosters (or depth-chart membership if rosters stay 403) as the pool for each team-week; absent-from-stats = 0 points. **Acceptance:** `evaluate_week` accepts a pool source; baseline re-measured on the full pool and recorded; leakage test green. *Prerequisite for validating T22 honestly.*
