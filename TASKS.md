...
- **T25. DST projection is blind to the spread — measured live, real effect size.** Caught on 2026-w02 DET@BUF: Lions (5.5-pt underdog) and Bills (5.5-pt favorite) projected almost identically (6.2 vs 6.3) because v4's DST projection is a role-bucket trailing mean with no knowledge of the week's line. Checked against 3,774 historical DST games: `dk_points ~ 6.09 + 0.233 x own_spread` (own_spread = home-favored-positive from the team's own perspective), r=0.27 — a large, real effect (favorites by 4-7 average 7.34, underdogs by 4-7 average 4.57, n=555 each). Fixed by hand for tonight via two committed overrides (see `data/projection_overrides.csv`); the real fix is a spread term in the DST branch of `RoleAware` (or a small dedicated DST model) so this doesn't need catching manually every lopsided line. *Blocks: nothing tonight; blocks trusting DST projections on any real spread going forward.*
