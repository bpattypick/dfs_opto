...
- **T15. Field generator needs joint structure, not a tuned `jitter`.** Matching marginal ownership cannot reproduce duplication, which is a joint property — see data-sources.md. At `jitter=0.15` the top player is 96% owned (absurd) but duplication appears; at 0.5 ownership looks plausible and duplication vanishes. No single value gives both, so this is not a calibration exercise. Candidate approaches: sample from a pool of *lineups* weighted by plausibility rather than filling slots from marginals, or add an explicit chalk-cluster component (a share of the field entering the same few optimal builds) on top of the diffuse sampler. **Do not start before H2 produces standings** — real ownership and real duplication counts are what say which shape is right, and guessing again is how the current default happened. *Blocks: trusting any duplication or dup-adjusted ROI number, including T6's.*
