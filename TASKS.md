...
- **T18. Shrink projections toward the mean — measured bias of -5.1 points.** Across players owned >15% on 2026-w01 SF@LAR, `AvgPointsPerGame` overshot actual scoring by 5.1 points on average, and the three highest-projected players missed by 11-17. It is regression to the mean: last season's per-game average selects for players who sustained a high rate, whom a new season pulls back. Shrink each projection toward a positional mean with a weight set by how many games the average rests on. Cheap, backtestable against `player_week_stats` 2019-2025, and it attacks the largest measured error in the pipeline. Pairs with T16 (too low on role changes) and T17 (too generous on minimum-priced depth) — all three are the same projection being wrong in different regimes.
- **T19. Captain selection must weight confidence, not just projected points.** The captain slot pays 1.5x, so it multiplies projection error by 1.5 — and because the optimizer captains whoever maximises projected points, it systematically captains the most over-projected player on the board, exactly where T18's bias is worst. On SF@LAR that turned a 33.2-point expected captain into 7.6 and cost the entry roughly half its score. Needs a floor/ceiling notion per player rather than a point estimate, so it depends on T13 or T18 landing first. *Blocks: trusting the optimizer's captain choice.*
